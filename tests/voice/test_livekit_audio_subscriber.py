...
class TestLiveKitAudioProcessorResilience:
    ...
    async def test_recovers_once_ffmpeg_becomes_available(self):
        """Verify process_frame recovers and yields audio after an initial failure."""
        processor = LiveKitAudioProcessor(output_sample_rate=16000)
        try:
            with patch.object(
    ...
    @pytest.mark.asyncio
    async def test_inmemory_resampler_error_degrades_gracefully(self):
        """Unexpected resampler errors log and return b'' gracefully."""
        processor = LiveKitAudioProcessor(output_sample_rate=16000)
        with patch.object(processor, "_inmemory_resample", new=AsyncMock(side_effect=Exception("resampler error"))):
            out = await processor.process_frame(_FRAME_48K_MONO, sample_rate=48000, num_channels=1)
        assert out == b""

    @pytest.mark.asyncio
    async def test_close_clears_resampler_state(self):
        """Verify close() resets the internal resampler reference."""
        processor = LiveKitAudioProcessor(output_sample_rate=16000)
