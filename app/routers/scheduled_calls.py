...
async def get_jira_field_map(
    ...
    """
    Get Jira field map for a specific project.
    Automatically ensures required fields exist and returns their mapping.
    ...
    Returns:
        {
            ...
            "field_map": {
                "status": "status",
                "agent_id": "customfield_10104",
                "call_time_utc": "customfield_10105",
                "tenant_id": "customfield_10106",
                "user_id": "customfield_10107",
                "batch_id": "customfield_10108",
                "call_session_id": "customfield_10109",
                "phone_number_id": "customfield_10110",
                "email_sent": "customfield_10111"
            }
        }
    """
    try:
        ...
        # Get or create field map (ensures fields exist)
        # This will automatically create missing fields if needed
        field_map = jira_service.ensure_required_fields(project_key)
        ...
        return create_success_response(
            result,
            f"Field map retrieved for project {project_key}"
        )
        
    except HTTPException:
        raise
    except ValueError as e:
        # Field creation or validation errors
        logger.error(f"❌ Field map error for project {project_key}: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Failed to get field map: {str(e)}"
        )
    except Exception as e:
        logger.error(f"❌ Failed to get field map for project {project_key}: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get field map: {str(e)}"
        )
