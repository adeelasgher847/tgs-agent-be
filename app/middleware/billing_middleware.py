...
class BillingMiddleware:
    ...
    async def _handle_agent_usage(self, request: Request, scope, receive, send):
        ...
        try:
            ...
            logger.error("Billing middleware error: %s", str(e), exc_info=True)
            await self.app(scope, receive, send)
...
def check_agent_creation_limit(db: Session, tenant_id: uuid.UUID) -> bool:
    ...
    try:
        ...
        logger.error("Error checking agent limit: %s", str(e), exc_info=True)
        return False
...
def check_calls_limit(db: Session, tenant_id: uuid.UUID, additional_calls: int = 1) -> bool:
    ...
    try:
        ...
        logger.error("Error checking calls limit: %s", str(e), exc_info=True)
        return False
...
def track_agent_creation(db: Session, tenant_id: uuid.UUID) -> None:
    ...
    try:
        ...
        logger.error("Error tracking agent creation: %s", str(e), exc_info=True)
...
def track_calls_usage(db: Session, tenant_id: uuid.UUID, calls_count: int = 1) -> None:
    ...
    try:
        ...
        logger.error("Error tracking calls usage: %s", str(e), exc_info=True)
...
def enforce_billing_limits(db: Session, tenant_id: uuid.UUID) -> dict:
    ...
    try:
        ...
        logger.error("Error enforcing billing limits: %s", str(e), exc_info=True)
