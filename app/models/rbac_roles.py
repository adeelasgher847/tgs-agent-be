...
class RbacRole(Base):
    ...
    __table_args__ = (
        Index("idx_rbacrole_workspace_user", "workspace_id", "user_id"),
        Index("uq_rbacrole_workspace_user_active", "workspace_id", "user_id", unique=True, postgresql_where=text("deleted_at IS NULL")),
        CheckConstraint(
            "role IN ('admin', 'manager', 'config_only', 'read_only', 'billing_only')",
            name="chk_rbacrole_role",
        ),
    )
