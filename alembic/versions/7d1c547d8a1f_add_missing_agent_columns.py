...
def upgrade() -> None:
    """Upgrade schema."""
    # This migration's name ("add_missing_agent_columns") and content suggest
    # it was written to patch a real database that had drifted from what the
    # branch being merged here expected — several of these columns/indexes
    # are also added (idempotently, in most cases) by other migrations on
    # the sibling branch. Guarded throughout so this is a true no-op once
    # those have already run, on a fresh install or otherwise.
    conn = op.get_bind()
    ...
    if not _has_column(conn, "agent", "stt_model_id"):
    ...
    if not _has_column(conn, "agent", "stt_settings_json"):
        op.add_column('agent', sa.Column('stt_settings_json', sa.JSON(), nullable=True))
    if _has_index(conn, "agent", "ix_agent_status"):
        op.drop_index(op.f('ix_agent_status'), table_name='agent')
    if not _has_index(conn, "agent", "ix_agent_stt_model_id"):
        op.create_index(op.f('ix_agent_stt_model_id'), 'agent', ['stt_model_id'], unique=False)
    if not _has_index(conn, "agent", "ix_agent_stt_provider_id"):
        op.create_index(op.f('ix_agent_stt_provider_id'), 'agent', ['stt_provider_id'], unique=False)
    if not _has_index(conn, "agent", "ix_agent_tenant_id"):
        op.create_index('ix_agent_tenant_id', 'agent', ['tenant_id'], unique=False)
    if not any(fk["constrained_columns"] == ["tts_voice_id"] for fk in inspect(conn).get_foreign_keys("agent")):
        op.create_foreign_key(None, 'agent', 'ttsvoice', ['tts_voice_id'], ['id'])
    if not any(fk["constrained_columns"] == ["tts_provider_id"] for fk in inspect(conn).get_foreign_keys("agent")):
        op.create_foreign_key(None, 'agent', 'ttsprovider', ['tts_provider_id'], ['id'])
    if not _has_index(conn, "callflow", "ix_callflow_agent_id"):
    ...
    if not _has_index(conn, "jobdescription", "ix_jobdescription_id"):
